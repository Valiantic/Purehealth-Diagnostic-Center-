                ...
                <table className="w-full table-fixed border-collapse text-sm">
                  <thead className="bg-gray-100 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className={`p-1 md:p-2 text-left border-b border-gray-200 font-bold text-green-800 text-sm sm:text-xs md:text-sm ${isEditingSummary ? 'w-[15%]' : 'w-[17%]'}`}>
                        <div className="flex items-center gap-1">
                          {(() => {
                            ...
                            const hasValue = testDetails?.some(test => parseFloat(test.cashAmount || 0) > 0) || false;
                            return hasValue ? (
                              <input
                                type="checkbox"
                                checked={true}
                          ...
                          <span>{isEditingSummary ? 'Cash Paid' : 'Cash Pay'}</span>
                        </div>
                      </th>
                      <th className={`p-1 md:p-2 text-left border-b border-gray-200 font-bold text-green-800 text-sm sm:text-xs md:text-sm ${isEditingSummary ? 'w-[15%]' : 'w-[17%]'}`}>
                        <div className="flex items-center gap-1">
                          {(() => {
                            const testDetails = isEditingSummary
                              ? editedTransaction?.originalTransaction?.TestDetails
                              : transaction?.originalTransaction?.TestDetails;
                            const hasValue = testDetails?.some(test => parseFloat(test.gCashAmount || 0) > 0) || false;
                            return hasValue ? (
                              <input
                                type="checkbox"
                                checked={true}
                      ...
                      <th className={`p-1 md:p-2 text-left border-b border-gray-200 font-bold text-green-800 text-sm sm:text-xs md:text-sm ${isEditingSummary && isRefundMode ? 'w-[14%]' : 'w-[16%]'}`}>
                        <div className="flex items-center gap-1">
                          {(() => {
                            const testDetails = isEditingSummary
                              ? editedTransaction?.originalTransaction?.TestDetails
                              : transaction?.originalTransaction?.TestDetails;
                            const hasValue = testDetails?.some(test => parseFloat(test.balanceAmount || 0) > 0) || false;
                            return hasValue ? (
                              <input
                                type="checkbox"
                                checked={true}
