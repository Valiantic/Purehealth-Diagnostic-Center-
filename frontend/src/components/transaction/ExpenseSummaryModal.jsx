...
const ExpenseSummaryModal = ({
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      // Debug log
      console.log(`About to update expense with ID ${expenseId}`);
      ...
      let formattedDate = editedExpense.date;
      if (editedExpense.date instanceof Date) {
        formattedDate = editedExpense.date.toISOString().split('T')[0];
      } else if (typeof editedExpense.date === 'string' && editedExpense.date.includes('T')) {
        formattedDate = editedExpense.date.split('T')[0];
      }
      console.log('Formatted expense date:', formattedDate);

      // Update status for items
      const updatedItems = editedExpense.ExpenseItems.map(item => {
        return {
          ...item,
          id: item.id || undefined, // Skip ID if it's not present
          paidTo: item.paidTo || '',
      ...
      console.log("Sending expense update with data:", updateData);
      
      // Use the centralized API service
      const response = await expenseAPI.updateExpenseDirectly(expenseId, updateData);
      
      toast.success('Expense updated successfully');
      
      // Call the onSave callback with the updated expense
      if (typeof onSave === 'function') {
        onSave({
          ...response.data?.data || editedExpense,
          date: formattedDate // Make sure the correct date is passed back
        });
      }
    } catch (error) {
            ...
            <div className="overflow-y-auto flex-1 scrollbar-hide"
              ...
              <div className="p-2 md:p-4 border-b border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-4">
                  {/* Payee Name - Takes full width on mobile, 1/3 on desktop */}
                  ...
                  {/* Department - Takes full width on mobile, 1/3 on desktop */}
                  <div className="md:col-span-1">
                  ...
                  {/* Date - Takes full width on mobile, 1/3 on desktop */}
                  <div className="md:col-span-1">
                    ...
                    {isEditing ? (
                      <div className="relative">
                        <input
                          ...
                          className="w-full px-2 md:px-3 py-1 md:py-2 border border-green-600 rounded cursor-pointer focus:outline-none focus:ring-1 focus:ring-green-600 text-xs md:text-base"
